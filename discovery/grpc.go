...
func ServiceConnection(ctx context.Context, serviceName string, registry Registry) (*grpc.ClientConn, error) {
	...
	// ⭐ Fallback: Direct service address if registry disabled (Kubernetes DNS)
	if registry == nil {
		// Default ports for services in Docker Compose / Kubernetes
		...
		if !ok {
			return nil, fmt.Errorf("unknown service %s (registry disabled)", serviceName)
		}
