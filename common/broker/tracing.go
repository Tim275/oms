...
// GetTraceIDFromAMQP: Extract Trace ID from AMQP headers for logging
// Warum?
// → Logs mit Trace IDs können in Jaeger nachgeschaut werden
// → Correlation zwischen AMQP Messages und Traces!
func GetTraceIDFromAMQP(headers amqp.Table) string {
	// W3C Trace Context format: "traceparent" header
	// Format: "00-<trace-id>-<span-id>-<flags>"
	if traceparent, ok := headers["traceparent"]; ok {
		if str, ok := traceparent.(string); ok && len(str) >= 55 {
			// Extract trace-id (characters 3-35)
			return str[3:35]
		}
	}
	return ""
