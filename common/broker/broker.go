...
// Warum?
// → AMQP hat KEINE automatische Trace-Propagation wie gRPC/HTTP
// → Wir müssen manuell den W3C Trace Context in die Headers packen
// → So kann der Consumer den Trace fortsetzen (same trace ID!)
func InjectAMQPHeaders(ctx context.Context) map[string]interface{} {
	carrier := make(AmqpHeaderCarrier)
	// Inject trace context (traceparent, tracestate) into AMQP headers
...
// Warum?
// → Consumer empfängt AMQP Message mit Trace Context in Headers
// → Wir extrahieren den Context und erstellen einen neuen Span
// → Trace wird fortgesetzt! Gateway → Orders → Payment → Stock/Kitchen (same trace ID!)
func ExtractAMQPHeader(ctx context.Context, headers map[string]interface{}) context.Context {
	if headers == nil {
		return ctx
	}
	// Extract trace context (traceparent, tracestate) from AMQP headers
