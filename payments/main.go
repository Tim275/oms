...
func main() {
	...
	// ✅ Pass ResilientConnection (not channel!) to HTTP Handler
	// → Handler will get fresh channels for each publish
	// → Auto-retry on channel failure
	httpServer := NewPaymentHTTPHandler(app.resilientConn, app.ordersGateway, cfg.OrdersAddr)
