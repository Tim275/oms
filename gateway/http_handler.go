...
func (h *handler) registerRoute(mux *http.ServeMux) {
	...
	// Wraps every handler with otelhttp.NewHandler for automatic tracing
	// → Creates Spans for each HTTP request
	// → Captures: method, path, status_code, duration
	// → Propagates trace context to downstream services (Orders, Stock, etc.)
	mux.Handle("POST /api/customers/{customerID}/orders",
		otelhttp.NewHandler(http.HandlerFunc(h.handleCreateOrder), "POST /api/customers/{customerID}/orders"))

	mux.Handle("GET /api/customers/{customerID}/orders/{orderID}",
		otelhttp.NewHandler(http.HandlerFunc(h.handleGetOrder), "GET /api/customers/{customerID}/orders/{orderID}"))

	mux.Handle("PUT /api/customers/{customerID}/orders/{orderID}",
		otelhttp.NewHandler(http.HandlerFunc(h.handleUpdateOrder), "PUT /api/customers/{customerID}/orders/{orderID}"))

	mux.Handle("GET /api/menu",
		otelhttp.NewHandler(http.HandlerFunc(h.handleGetMenu), "GET /api/menu"))

	mux.Handle("GET /api/orders",
		otelhttp.NewHandler(http.HandlerFunc(h.handleGetOrders), "GET /api/orders"))

	mux.Handle("GET /api/customers/{customerID}/orders/{orderID}/receipt",
		otelhttp.NewHandler(http.HandlerFunc(h.handleGetReceipt), "GET /api/customers/{customerID}/orders/{orderID}/receipt"))

	// Serve static files from public directory (no tracing needed for static files)
...
func (h *handler) handleGetOrder(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.logger.Error("failed to discover orders service",
			slog.Any("error", err),
...
// handleGetReceipt: GET /api/customers/{customerID}/orders/{orderID}/receipt
// Returns a receipt/pickup slip for the order (Abholschein like McDonald's)
func (h *handler) handleGetReceipt(w http.ResponseWriter, r *http.Request) {
	customerID := r.PathValue("customerID")
	orderID := r.PathValue("orderID")

	h.logger.Info("get receipt request",
		slog.String("customer_id", customerID),
		slog.String("order_id", orderID),
	)
	...
	// Convert items
	receiptItems := make([]ReceiptItem, len(order.Items))
	...
	receipt := Receipt{
		...
		Message:     "Bitte diese Nummer vorzeigen beim Abholen / Show this number when picking up your order",
	}

	h.logger.Info("receipt generated successfully",
		slog.String("order_id", orderID),
		slog.Int("order_number", int(order.OrderNumber)),
	)
