...
func NewGRPCHandler(grpcServer *grpc.Server, service OrdersService, store OrdersStore, resilientConn *broker.ResilientConnection, logger *slog.Logger, registry discovery.Registry) {
	handler := &grpcHandler{
		service:       service,
		store:         store,
...
func (h *grpcHandler) CreateOrder(ctx context.Context, req *api.CreateOrderRequest) (*api.Order, error) {
	...
	// Warum resilientConn == nil Check?
	// → RabbitMQ ist OPTIONAL! Service funktioniert auch OHNE Events
	// → Bei Tests oder Entwicklung: Kein RabbitMQ → resilientConn = nil
	if h.resilientConn == nil {
		h.logger.Error("rabbitmq connection is nil, event not published")
		return order, nil  // Return order anyway! (Event Publishing ist nicht kritisch)
	}

	// Warum json.Marshal ZUERST?
	// → Wir marshallen BEVOR wir Channel holen
	// → Falls Marshal fehlschlägt: Kein unnötiger Channel Request!
	...
	// ⭐ RETRY LOGIC mit fresh channel bei jedem Versuch
	// Warum Retry?
	// → Channel kann zwischen Channel() und Publish() schließen
	// → In Kubernetes: Network issues, Pod restarts, etc.
	// → 3 Versuche mit fresh channel = Production ready!
	maxRetries := 3
	var publishErr error

	for attempt := 1; attempt <= maxRetries; attempt++ {
		// Get fresh channel from ResilientConnection
		// → Neuer Channel bei JEDEM Versuch!
		ch, err := h.resilientConn.Channel()
		if err != nil {
			h.logger.Warn("failed to get channel",
		...
		q, err := ch.QueueDeclare(
			...
			amqp.Table{
				"x-dead-letter-exchange": broker.DLX, // DLX Integration!
			},
		)
		if err != nil {
			h.logger.Warn("failed to declare queue",
				slog.String("queue", broker.OrderCreatedEvent),
				slog.Int("attempt", attempt),
				slog.Any("error", err),
			)
			publishErr = err
			continue // Try again with new channel
		}
		...
		err = ch.PublishWithContext(
			...
			amqp.Publishing{
				...
				Headers:     broker.InjectTraceContext(ctx), // ⭐ OpenTelemetry!
			},
		)
		if err != nil {
	...
	if publishErr != nil {
		h.logger.Error("failed to publish event after retries",
			slog.String("event", broker.OrderCreatedEvent),
			slog.String("order_id", order.Id),
...
func (h *grpcHandler) UpdateOrder(ctx context.Context, req *api.Order) (*api.Order, error) {
	...
	if previousOrder.Status != updatedOrder.Status && h.resilientConn != nil {
