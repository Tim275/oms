...
func NewStore(client *mongo.Client) *store {
	...
	countersCollection := client.Database("orders").Collection("counters")
	return &store{
...
// getNextOrderNumber atomically increments and returns the next order number
func (s *store) getNextOrderNumber(ctx context.Context) (int32, error) {
	filter := bson.M{"_id": "order_number"}
	update := bson.M{"$inc": bson.M{"seq": 1}}

	// Create if doesn't exist, increment if exists
	...
	err := s.countersCollection.FindOneAndUpdate(ctx, filter, update, opts).Decode(&result)
	if err != nil {
		return 0, err
	}
...
func (s *store) Create(ctx context.Context, order *api.Order) (primitive.ObjectID, error) {
	// Generate next order number (1, 2, 3, 4... like McDonald's)
	orderNumber, err := s.getNextOrderNumber(ctx)
	if err != nil {
...
func (s *store) Get(ctx context.Context, orderID string) (*api.Order, error) {
	...
	order := &api.Order{
		...
		CreatedAt:   createdAt,
		OrderNumber: getInt32(doc, "orderNumber"),
	}
